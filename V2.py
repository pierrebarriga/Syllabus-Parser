...
def parse_syllabus_with_ai(raw_text, api_key):
...
if uploaded_file is not None:
    if not user_api_key:
        ...
        with st.spinner("Step 1: Extracting text from PDF..."):
            ...
            st.info(f"Successfully extracted {len(raw_text)} characters.")
            
        with st.spinner("Step 2: AI is analyzing structure..."):
            try:
                json_raw_output = parse_syllabus_with_ai(raw_text, user_api_key)
